...
import type { CardWrite, JsonRecord, SourceInfo } from "./types.js";
...
function output(value: Record<string, unknown>, format: "markdown" | "json", markdown: string) {
  return {
    content: [{ type: "text" as const, text: format === "json" ? JSON.stringify(value, null, 2) : markdown }],
...
server.registerTool("collector_get_target", {
  ...
  if (!target) return output({ found: false, target_id }, response_format, `Target not found: ${target_id}. Call collector_list_targets or collector_create_target.`);
...
server.registerTool("collector_upsert_cards", {
  ...
  try {
    ...
    const operation = db.runOperation(operation_id, "cards.upserted", `${source.agent}:${source.tool}`, (data) => {
      const normalizedSource = { ...source, run_id: runId } as SourceInfo;
      const changed = upsertCards(data, target_id, normalizedSource, cards as CardWrite[]);
      return {
    ...
    return { isError: true, content: [{ type: "text" as const, text: `Unable to write cards: ${error instanceof Error ? error.message : String(error)}` }] };
  }
});
...
server.registerTool("collector_add_connection", {
  ...
  try {
    ...
    return { isError: true, content: [{ type: "text" as const, text: `Unable to create connection: ${error instanceof Error ? error.message : String(error)}` }] };
  }
});
