...
import type { ActivityRow, Card, JsonRecord, Target, WorkspaceData, WorkspaceState } from "./types.js";

export class RevisionConflictError extends Error {
  constructor(public readonly current: WorkspaceState) {
...
export class WorkbenchDatabase {
  ...
  getRevision(): number {
    const row = this.db.prepare("SELECT revision FROM workspace_state WHERE id = 1").get() as { revision: number } | undefined;
    return row?.revision ?? 0;
  }

  getWorkspace(): WorkspaceState {
    const row = this.db.prepare("SELECT revision, data_json, settings_json, updated_at FROM workspace_state WHERE id = 1").get() as {
      revision: number; data_json: string; settings_json: string; updated_at: string;
    } | undefined;
    if (!row) return { exists: false, revision: 0, data: null, settings: null, updated_at: null };
    return {
      exists: true,
      revision: row.revision,
      data: normalizeWorkspace(JSON.parse(row.data_json) as unknown),
      settings: JSON.parse(row.settings_json) as JsonRecord,
      updated_at: row.updated_at
    };
  }

  saveWorkspace(dataValue: unknown, settingsValue: unknown, expectedRevision: number, source = "browser"): WorkspaceState {
    const data = normalizeWorkspace(dataValue);
    const settings = settingsValue && typeof settingsValue === "object" ? settingsValue as JsonRecord : {};
  ...
  runOperation<T extends JsonRecord>(
    operationId: string,
    kind: string,
    source: string,
    mutate: (data: WorkspaceData) => { result: T; message: string; scanRun?: { run_id: string; agent: string; tool: string; target_id: string; scanned_at: string; card_count: number; metadata: JsonRecord } }
  ): OperationResult<T> {
    this.db.exec("BEGIN IMMEDIATE");
    try {
      const existing = this.db.prepare("SELECT result_json, revision FROM operations WHERE operation_id = ?").get(operationId) as { result_json: string; revision: number } | undefined;
      if (existing) {
        this.db.exec("COMMIT");
        return { duplicate: true, revision: existing.revision, result: JSON.parse(existing.result_json) as T };
      }
      const current = this.getWorkspace();
  ...
  private persistWorkspace(data: WorkspaceData, settings: JsonRecord, revision: number): void {
    const now = new Date().toISOString();
    ...
    data.targets.forEach((target, targetIndex) => {
      ...
      target.cards.forEach((card) => insertCard.run(
        card.id, target.id, card.mcpKey ?? null, card.title, card.templateId ?? null,
        card.data ?? "", card.risk ?? "info", card.status ?? "todo", JSON.stringify(card.tags ?? []),
        card.mcpSource ? JSON.stringify(card.mcpSource) : null, toIso(card.updatedAt ?? card.createdAt)
      ));
      (target.connections ?? []).forEach((connection) => insertConnection.run(
        connection.id, target.id, connection.source, connection.target, connection.type ?? "related", now
      ));
    });
  }

  private insertActivity(kind: string, message: string, source: string | null, revision: number): void {
  ...
  listActivity(limit = 30, offset = 0): { total_count: number; items: ActivityRow[] } {
    const total = (this.db.prepare("SELECT COUNT(*) AS count FROM activity").get() as { count: number }).count;
    const items = this.db.prepare("SELECT id, kind, message, source, created_at, revision FROM activity ORDER BY id DESC LIMIT ? OFFSET ?")
      .all(limit, offset) as unknown as ActivityRow[];
    return { total_count: total, items };
  }

  listRuns(limit = 20, offset = 0, tool = ""): { total_count: number; items: JsonRecord[] } {
    const where = tool ? "WHERE tool = ?" : "";
    const countArgs = tool ? [tool] : [];
    const total = (this.db.prepare(`SELECT COUNT(*) AS count FROM scan_runs ${where}`).get(...countArgs) as { count: number }).count;
    const args: Array<string | number> = tool ? [tool, limit, offset] : [limit, offset];
    ...
    return { total_count: total, items: rows as unknown as JsonRecord[] };
  }

  searchCards(query: string, targetId: string, risk: string, tag: string, limit: number, offset: number): { total_count: number; items: JsonRecord[] } {
    const clauses: string[] = [];
    const args: Array<string | number> = [];
    if (query) { clauses.push("(c.title LIKE ? ESCAPE '\\' OR c.content LIKE ? ESCAPE '\\')"); const like = `%${escapeLike(query)}%`; args.push(like, like); }
    if (targetId) { clauses.push("c.target_id = ?"); args.push(targetId); }
    if (risk) { clauses.push("c.risk = ?"); args.push(risk); }
    if (tag) { clauses.push("EXISTS (SELECT 1 FROM json_each(c.tags_json) WHERE value = ?)"); args.push(tag); }
    const where = clauses.length ? `WHERE ${clauses.join(" AND ")}` : "";
    const total = (this.db.prepare(`SELECT COUNT(*) AS count FROM cards c ${where}`).get(...args) as { count: number }).count;
    const rows = this.db.prepare(`SELECT c.id, c.target_id, t.name AS target_name, c.mcp_key, c.title, c.template_id, c.content, c.risk, c.status, c.tags_json, c.source_json, c.updated_at
      FROM cards c JOIN targets t ON t.id = c.target_id ${where} ORDER BY c.updated_at DESC LIMIT ? OFFSET ?`).all(...args, limit, offset) as Array<Record<string, unknown>>;
