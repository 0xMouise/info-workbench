...
test("v9 MCP writes directly to SQLite and exposes read-only discovery tools", async () => {
    ...
    const env = Object.fromEntries(Object.entries(process.env).filter((item) => typeof item[1] === "string"));
    ...
    try {
        ...
        assert.equal(written.structuredContent?.revision, 2);
        ...
        assert.equal(search.structuredContent?.total_count, 1);
    }
    finally {
        await client.close();
