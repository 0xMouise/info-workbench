...
const server = http.createServer(async (req, res) => {
    try {
        if (!validHost(req) || !validOrigin(req))
            return json(res, 403, { error: "Local workbench origin required" });
        ...
        if (req.method === "GET" && url.pathname === "/api/workspace")
            return json(res, 200, db.getWorkspace());
        if (req.method === "PUT" && url.pathname === "/api/workspace") {
            const body = await readBody(req);
            if (!isWorkspacePayload(body))
                return json(res, 400, { error: "Invalid workspace payload" });
            try {
                ...
                if (error instanceof RevisionConflictError)
                    return json(res, 409, { error: error.message, current: error.current });
                throw error;
            }
        }
...
server.on("upgrade", (req, socket, head) => {
    const url = new URL(req.url ?? "/", `http://${host}:${port}`);
    if (url.pathname !== "/ws" || !validHost(req) || !validOrigin(req))
...
function broadcastRevision(revision) {
    ...
    sockets.forEach((socket) => { if (socket.readyState === WebSocket.OPEN)
        socket.send(message); });
}
setInterval(() => {
    try {
